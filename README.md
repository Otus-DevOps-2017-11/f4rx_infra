...
В **packer_db.yaml** мой эсперемент, чтобы не указывать **-name**. Считаю лишним писать какое-то пояснение типа "создаю 
...
Сам скрипт доступен в офф рефе.
```bash
...
модуль поддерживает три режима авторижзации - создания secret.py, ini-файла или переменных окружения.  Я сделал через
